...
public class SwerveSubsystem extends SubsystemBase {
    ...
    ProfiledPIDController headingPID =
        new ProfiledPIDController(Constants.AutoConstants.kPThetaController, 0.0, 0.0,
    ...
    public SwerveSubsystem() {
        ...
        swerveModules = new SwerveModule[] {new SwerveModule(0, Constants.Swerve.Mod0.constants),
        ...
        poseEstimator = new SwerveDrivePoseEstimator(Constants.Swerve.swerveKinematics, getYaw(),
            getModulePositions(), new Pose2d(), odoStdDevs, visStdDevs);
    ...
    /**
     * Set the modules to the correct state based on a desired translation and rotation, either
     * field or robot relative and either open or closed loop
     */
    ...
    public void drive(ChassisSpeeds chassisSpeeds, boolean isOpenLoop) {
        drive(new Translation2d(chassisSpeeds.vxMetersPerSecond, chassisSpeeds.vyMetersPerSecond),
            chassisSpeeds.omegaRadiansPerSecond, false, isOpenLoop);
    }
    ...
    public Command driveCommand(DoubleSupplier x, DoubleSupplier y, DoubleSupplier theta,
        ...
        return new RunCommand(() -> drive(
            new Translation2d(x.getAsDouble() * x.getAsDouble() * Math.signum(x.getAsDouble()),
                y.getAsDouble() * y.getAsDouble() * Math.signum(y.getAsDouble()))
                    .times(Constants.Swerve.maxSpeed),
            theta.getAsDouble() * Constants.Swerve.maxAngularVelocity, fieldRelative, isOpenLoop),
            this);
    }

    public Command drivePIDHeadingCommand(DoubleSupplier x, DoubleSupplier y,
        DoubleSupplier targetTheta, boolean fieldRelative, boolean isOpenLoop) {
        return new RunCommand(() -> drive(
            new Translation2d(x.getAsDouble(), y.getAsDouble()).times(Constants.Swerve.maxSpeed),
            headingPID.calculate(getYaw().getRadians(), targetTheta.getAsDouble()), fieldRelative,
            isOpenLoop), this);
    }
    ...
    public Command followPathCommand(PathPlannerTrajectory path) {
        return new SwerveControllerCommand(path, () -> getPose(), Constants.Swerve.swerveKinematics,
            new HolonomicDriveController(
                ...
                new ProfiledPIDController(Constants.AutoConstants.kPThetaController, 0, 0,
                    Constants.AutoConstants.thetaControllerConstraints)),
            (states) -> {
                drive(Constants.Swerve.swerveKinematics.toChassisSpeeds(states), true);
            }, this);
    }

    /**
     * This function optimizes the chassis speed that is put into the kinematics object to allow the
     * robot to hold its heading when no angular velocity is input. The robot will therefore correct
     * itself when it turns without us telling it to do so. Made by 4481, used to correct drift from
     * first order kinematics
     *
     * @param desiredSpeed Desired chassis speed that is input by the controller
     * @return {@code correctedChassisSpeed} which takes into account that the robot needs to have
     *         the same heading when no rotational speed is input
     */
    private ChassisSpeeds correctHeading(ChassisSpeeds desiredSpeed) {
        ...
        // Get desired rotational speed in radians per second and absolute translational speed in
        // m/s
        double vr = desiredSpeed.omegaRadiansPerSecond;
        double v = Math.sqrt(Math.pow(desiredSpeed.vxMetersPerSecond, 2)
            + Math.pow(desiredSpeed.vxMetersPerSecond, 2));
        ...
        double correctedVr =
            deltaHeading.getRadians() / dt * Constants.AutoConstants.kPThetaController;

        previousTime = currentTime;

        return new ChassisSpeeds(desiredSpeed.vxMetersPerSecond, desiredSpeed.vyMetersPerSecond,
            correctedVr);
    }

    /** 4481 second order swerve module updating */
    public void updateSwerveModuleStates(ChassisSpeeds desiredSpeeds, boolean isOpenLoop,
        ...
        // The second order kinematics will return swerveModuleStates and desired rotational speeds
        // of the turn motors
        // Both the swerveModuleStates and the desired turn velocity are saved in the
        // secondOrderSwerveModuleState object
        SecondOrderSwerveModuleStates secondOrderSwerveModuleStates =
            Constants.Swerve.secondSwerveKinematics.toSwerveModuleState(desiredSpeedsFieldRelative,
                getYaw());
        SwerveModuleState[] swerveModuleStates =
            secondOrderSwerveModuleStates.getSwerveModuleStates();
        ...
        for (SwerveModule mod : swerveModules) {
            mod.setDesiredState(swerveModuleStates[mod.moduleNumber],
                moduleTurnSpeeds[mod.moduleNumber], isOpenLoop);
        }
    }
    ...
    /**
     * Updates the pose estimator from a (presumably vision) measurement Input is in the form of a
     * list of pose2ds and a latency measurement
     */
        ...
        if (data != null) {
            ...
            for (Pose2d pose : data.getFirst()) {
                ...
                poseEstimator.addVisionMeasurement(pose,
                    Timer.getFPGATimestamp() - (data.getSecond() / 1000));
        ...
        if (result.hasTargets()) {
            ...
            for (PhotonTrackedTarget target : result.getTargets()) {
                ...
                if (target.getPoseAmbiguity() < 0.1) {
                    ...
                    poses.add(getFieldToRobot(targetPose3d, Constants.CAMERA_TO_ROBOT,
                        target.getBestCameraToTarget()).toPose2d());
                }
    ...
    /**
     * Estimates the pose of the robot in the field coordinate system, given the id of the fiducial,
     * the robot relative to the camera, and the target relative to the camera. Stolen from
     * mdurrani834
     *
     * @param tagPose Pose3d the field relative pose of the target
     * @param robotToCamera Transform3d of the robot relative to the camera. Origin of the robot is
     ...
     */
    private Pose3d getFieldToRobot(Pose3d tagPose, Transform3d robotToCamera,
