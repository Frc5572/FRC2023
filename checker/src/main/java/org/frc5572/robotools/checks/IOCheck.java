...
public class IOCheck implements Check {
    ...
    private void checkType(CompilationData data, Element warnPos, String path, TypeElement elem, Set<String> skip) {
        ...
        for (String iface : ioInterfaces) {
            if (data.implementsInterface(elem, iface)) {
                ...
                data.warn(warnPos,
                        path + " is a " + parts[parts.length - 1] + " which performs IO. It should be in an IO class!");
                return;
            }
        }
        ...
        for (var item : elem.getEnclosedElements()) {
            if (item instanceof VariableElement) {
                ...
                if (e instanceof TypeElement) {
                    ...
                    checkType(data, warnPos, path2 + "." + variable.getSimpleName().toString(), type, skip);
                }
            }
        }
    }
